...
//define parent and child addresses to be set at specific addresses (arbitrary)
...
void tx_callback(uint8_t* data, uint8_t* len) {
  //parent stored at address 0x08
  *len = 1;
  data[0] = eeprom_read_byte((uint8_t*)parent_address);//set data to parent counter
  eeprom_update_byte((uint8_t*)parent_address,data[0]+1);//parent_counter+=1
  print("Parent counter incremented\n");

  uint8_t parent_read = eeprom_read_byte((uint8_t*)parent_address);
  print("parent_counter: %d\n", parent_read);
}

void rx_callback(uint8_t* data, uint8_t len) {
  //child counter stored at address 0x00
  print("TX received!\n");
  if (len != 0) {
    eeprom_update_byte((uint8_t*)child_address,data[0]);//child_counter = data[0];
    uint8_t child_read = eeprom_read_byte((uint8_t*)child_address);
